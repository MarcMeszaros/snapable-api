...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding field 'AccountAddon.account'
        db.add_column('data_accountaddon', 'account',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['data.Account']),
                      keep_default=False)

        if not db.dry_run:
            for accountaddon in orm.AccountAddon.objects.all():
                accountaddon.account = accountaddon.event.account
                accountaddon.save()

        # Deleting field 'AccountAddon.event'
        db.delete_column('data_accountaddon', 'event_id')

    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'AccountAddon.event'
        raise RuntimeError("Cannot reverse this migration. 'AccountAddon.event' and its values cannot be restored.")
    ...
    models = {
        ...
        'data.accountaddon': {
            ...
            'account': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['data.Account']", 'null': 'True'}),
