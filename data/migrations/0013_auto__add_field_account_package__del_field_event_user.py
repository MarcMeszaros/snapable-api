...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Account.package'
        db.add_column('data_account', 'package',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=1, to=orm['data.Package']),
                      keep_default=False)

        if not db.dry_run:
            for event in orm.Event.objects.all():
                account = event.account
                account.package = event.package
                account.save()

        # Deleting field 'Event.package'
        db.delete_column('data_event', 'package_id')
