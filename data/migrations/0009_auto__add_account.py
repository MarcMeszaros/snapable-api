...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table('data_account', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('admin', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['data.User'])),
        ))
        db.send_create_signal('data', ['Account'])

        if not db.dry_run:
            for user in orm.User.objects.all():
                account = orm.Account(id=user.id,admin_id=user.id)
                account.save()
