...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # delete all data from silly users with '' event urls
        if not db.dry_run:
            non_unique_urls = (orm.Event.objects.values_list('url', flat=True).annotate(url_count=models.Count('url')).filter(url_count__gt=1))
            for url in non_unique_urls:
                events = orm.Event.objects.filter(url=url)
                for event in events:
                    try:
        ...
        # Adding unique constraint on 'Event', fields ['url']
        db.create_unique('data_event', ['url'])
